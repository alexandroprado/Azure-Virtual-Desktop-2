...
# Configuring paramaters
$imageVersionParameters = @{
    GalleryImageDefinitionName = "AVD-PL"
    GalleryImageVersionName    = $version
    GalleryName                = "SIG"
    ResourceGroupName          = "SIG"
    Location                   = $location
...
# Configuring paramaters
$imageVersionParameters = @{
    GalleryImageDefinitionName = "AVD-RO"
    GalleryImageVersionName    = $version
    GalleryName                = "SIG"
    ResourceGroupName          = "SIG"
    Location                   = $location
...
# Configuring paramaters
$imageVersionParameters = @{
    GalleryImageDefinitionName = "AVD-UA"
    GalleryImageVersionName    = $version
    GalleryName                = "SIG"
    ResourceGroupName          = "SIG"
    Location                   = $location
...
$deploy = new-AzresourcegroupDeployment -TemplateUri "C:\Users\DennisWesterman\OneDrive - Westerman.it\_Azure\AVD\PS\AddSessionHostTemplate.json" @templateParameters -Name "deploy-version-$($latestImageVersion.Name)"
...
$sessionHosts = Get-AzWvdSessionHost -ResourceGroupName $resourceGroup -HostPoolName $hostpool.name
# Get latest sessionhost
$sessionHost1 = $sessionHosts[1]
$sessionHostName1 = $sessionHost1.name.Split("/")[-1]

Write-Host "Set $sessionHostName1 on Drain mode"
Update-AzWvdSessionHost -HostPoolName $Hostpoolname -ResourceGroupName $ResourceGroup -Name $sessionHostName1 -AllowNewSession:$false
Clear-Host
##############################################################################################################################################################
$hostpoolName = "AVD-RO"
# Get hostpool information
